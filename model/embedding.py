...
class FineTunableEmbedding(AbstractEmbedding):
    ...
    def forward_loss(self, x):
        ...
        x_frozen = self.invoke(self.frozen_emb, x_frozen)
        x_unfrozen = self.invoke(self.unfrozen_emb, x_unfrozen)
        return x_frozen + x_unfrozen

    @property
    def weight(self):
        idx = torch.arange(self.num_embeddings, requires_grad=False,
                           device=self.frozen_map.device)
        idx = idx.long()
        return self.forward_loss(idx)
