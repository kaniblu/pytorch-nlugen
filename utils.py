...
def to_device(mdl, devices):
    ...
    if len(devices) == 1:
        ...
        main_dev = devices[0]
        mdl = mdl.to(main_dev)
        mdl = nn.DataParallel(mdl, [d.index for d in devices],
...
def get_namespace_dict(parser, args):
    arg_groups = {}
...
def save_args(parser, args, f):
    gargs = get_namespace_dict(parser, args)
...
def parse_args(parser):
    ...
    if unk_args:
        print(f"[Warning] some arguments are unrecognized: {unk_args}")
    return args
...
def initialize_script(parser):
    ...
    argdict = {k: vars(v) for k, v in get_namespace_dict(parser, args).items()}
    logging.info(dump_yaml(argdict))
    return args
...
def dictify(kvps, reduce=latter):
    ret = dict()
    for k, v in kvps:
        if k not in ret:
            ret[k] = reduce(None, v)
        else:
            ret[k] = reduce(ret[k], v)
    return ret


def import_module(pkg_name):
...
def filter_namespace_prefix(args, prefix, delim="_"):
    argdict = vars(args)
    argdict = {k[len(prefix) + 1:]: v for k, v in argdict.items()
               if k.startswith(f"{prefix}{delim}")}
    return argparse.Namespace(**argdict)
